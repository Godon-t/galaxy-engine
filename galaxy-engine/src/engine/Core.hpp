...
#define GLX_ERROR
    ...
    #define GLX_ASSERT(x, ...)        { if(!(x)) { GLX_ERROR("Assertion failed: {0}", __VA_ARGS__); GLX_DEBUGBREAK(); } }
    #define GLX_CORE_ASSERT(x, ...)   { if(!(x)) { GLX_CORE_ERROR("Assertion failed: {0}", __VA_ARGS__); GLX_DEBUGBREAK(); } }
#else
    #define GLX_ASSERT(x, ...)
